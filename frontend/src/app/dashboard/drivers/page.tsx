...
const FAKE_DRIVERS = Array.from({ length: 12 }).map((_, i) => ({
  ...
  status: i % 4 === 0 ? "active" : i % 4 === 1 ? "offline" : i % 4 === 2 ? "on_trip" : "maintenance",
...
export default function DriversPage() {
  ...
  const statusConfig = {
    ...
    on_trip: { label: "On Trip", color: "bg-blue-100 text-blue-700", icon: Navigation },
    maintenance: { label: "Maintenance", color: "bg-orange-100 text-orange-700", icon: AlertCircle },
  };
  ...
  const filteredDrivers = (error || drivers.length === 0 ? FAKE_DRIVERS : drivers)
    .filter(driver => 
      (driver.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ?? true) &&
      (selectedStatus === "all" || driver.status === selectedStatus) &&
      (selectedVehicleType === "all" || driver.vehicle_type === selectedVehicleType)
    );
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-4xl font-bold text-gray-900 mb-2">Drivers Management</h1>
                <p className="text-gray-600 text-lg">Manage and monitor your fleet drivers</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            {[
              { label: "Total Drivers", value: stats.total, icon: Users, color: "from-blue-500 to-indigo-600" },
              { label: "Active", value: stats.active, icon: CheckCircle, color: "from-green-500 to-emerald-600" },
              { label: "On Trip", value: stats.onTrip, icon: Navigation, color: "from-purple-500 to-pink-600" },
              { label: "Offline", value: stats.offline, icon: XCircle, color: "from-gray-500 to-slate-600" },
            ].map((stat, index) => {
        ...
        {loading ? (
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-3 text-lg text-gray-600">Loading drivers...</span>
            </div>
          </div>
        ) : error ? (
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Error Loading Drivers</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDrivers.map((driver, index) => {
              const StatusIcon = statusConfig[driver.status as keyof typeof statusConfig]?.icon || CheckCircle;
              const VehicleIcon = vehicleTypeConfig[driver.vehicle_type as keyof typeof vehicleTypeConfig]?.icon || Car;
              ...
              return (
                <div
                  ...
                  onClick={() => !error && drivers.length > 0 && driver.id && router.push(`/dashboard/drivers/${driver.id}`)}
                >
                  {/* Driver Header */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-gray-900 text-lg">{driver.full_name}</h3>
                        <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${statusConfig[driver.status as keyof typeof statusConfig]?.color}`}>
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${vehicleTypeConfig[driver.vehicle_type as keyof typeof vehicleTypeConfig]?.color}`}>
                        {driver.vehicle_type}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-gray-50 rounded-xl p-3">
                      ...
                      <div className="text-lg font-bold text-gray-900">{driver.total_trips}</div>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3">
